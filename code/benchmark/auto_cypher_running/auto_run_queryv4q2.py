...
if __name__ == "__main__":
    ...
    while batch > 0:
        ...
        subprocess.call(['sh','./mv4_once.sh'])
        ...
        try:
            ...
            graph.run('CREATE INDEX meas FOR (m:Measurement) ON (m.measurement_id);')
            graph.run('CREATE INDEX drug FOR (d:DrugExposure) ON (d.drug_exposure_id);')
            graph.run('CREATE INDEX visit FOR (v:VisitOccurrence) ON (v.visit_occurrence_id);')
            graph.run('CREATE INDEX obs FOR (o:Observation) ON (o.observation_id);')
            graph.run('CREATE INDEX cond FOR (c:Condition) ON (c.condition_occurrence_id);')
            graph.run('CREATE INDEX proc FOR (p:ProcedureOccurrence) ON (p.procedure_occurrence_id); ')               
            ...
            while run_times > 0:
                ...
                print("-----Running question2 on mv4.-----")
    ...
    agg.to_csv('query_running_timev4q2.csv', index=False, header=header_list)
